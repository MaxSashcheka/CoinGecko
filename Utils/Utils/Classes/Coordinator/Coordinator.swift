 ...
 */
open class Coordinator: NSObject, Container {
    ...
    open var baseViewController: UIViewController {
        fatalError("CoinGeckoUtils.Coordinator.baseViewController\n" +
    ...
    open var activeViewController: UIViewController? {
        fatalError("CoinGeckoUtils.Coordinator.activeViewController\n" +
    ...
    open func add(child: Coordinator) {
        ...
        if child.parent != nil {
            assertionFailure("CoinGeckoUtils.Coordinator.add(child:)\n" +
    ...
    open func removeFromParent() {
        ...
        guard let index = parentCoordinator.children.firstIndex(where: { $0 === self }) else {
            assertionFailure("CoinGeckoUtils.Coordinator.removeFromParent()\n" +
    ...
    open func presentModal(coordinator: Coordinator,
        ...
        guard modallyShownCoordinator == nil && modallyShownController == nil else {
            assertionFailure("CoinGeckoUtils.Coordinator.presentModal(coordinator:presentationStyle:animated:completion:)\n" +
    ...
    open func dismissModalCoordinator(animated: Bool = true, completion: (() -> Void)? = nil) {
        if modallyShownCoordinator == nil {
            assertionFailure("CoinGeckoUtils.Coordinator.dismissModalCoordinator(animated:completion:)\n" +
    ...
    open func dismissModalController(animated: Bool = true, completion: (() -> Void)? = nil) {
        if modallyShownCoordinator != nil {
            assertionFailure("CoinGeckoUtils.Coordinator.dismissModalController(animated:completion:)\n" +
...
extension Coordinator: UIAdaptivePresentationControllerDelegate {
    public func presentationControllerShouldDismiss(_ presentationController: UIPresentationController) -> Bool {
        guard modallyShownCoordinator?.modallyShownCoordinator == nil else {
            assertionFailure("CoinGeckoUtils.Coordinator.presentationControllerShouldDismiss(_:)\n" +
                "Attempt to dismiss coordinator witch already has own modal coordinator. Dismiss it first")
        ...
        guard modallyShownController != nil
            ...
            assertionFailure("CoinGeckoUtils.Coordinator.Coordinator.presentationControllerShouldDismiss(_:)\n" +
    ...
    public func presentationControllerDidDismiss(_ presentationController: UIPresentationController) {
        guard presentationController.presentedViewController === modallyShownController else {
            assertionFailure("CoinGeckoUtils.Coordinator.Coordinator.presentationControllerDidDismiss(_:)\n" +
